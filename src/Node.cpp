...
namespace noco
{
	...
	namespace {
		std::shared_ptr<ComponentBase> CreateComponentFromJSON(const JSON& json)
		{
			...
			if (type == U"Label")
			{
				auto labelComponent = std::make_shared<Label>();
				if (!labelComponent->tryReadFromJSON(json))
				{
			...
			else if (type == U"Sprite")
			{
				...
				if (!spriteComponent->tryReadFromJSON(json))
				{
			...
			{
				auto rectComponent = std::make_shared<RectRenderer>();
				if (!rectComponent->tryReadFromJSON(json))
				{
			...
			{
				auto textBoxComponent = std::make_shared<TextBox>();
				if (!textBoxComponent->tryReadFromJSON(json))
				{
			...
			{
				auto inputBlockerComponent = std::make_shared<InputBlocker>();
				if (!inputBlockerComponent->tryReadFromJSON(json))
				{
					Logger << U"[NocoUI warning] Failed to read InputBlocker component from JSON";
					return nullptr;
				}
				component = inputBlockerComponent;
			}
			else if (type == U"EventTrigger")
			{
				auto eventTriggerComponent = std::make_shared<EventTrigger>();
				if (!eventTriggerComponent->tryReadFromJSON(json))
				{
			...
			{
				auto placeholderComponent = std::make_shared<Placeholder>();
				if (!placeholderComponent->tryReadFromJSON(json))
				{
