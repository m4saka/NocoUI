...
private:
	...
	CustomAssetLoader m_customAssetLoader;
	bool m_assetDirectoryManuallySet = false;
...
public:
		...
		, m_prevSceneSize(Scene::Size())
	{
		...
		m_menuBar.addMenuCategory(
			...
			KeyT,
			{
				...
				MenuItem{ U"アセットを再読み込み", U"", KeyR, [this] { onClickMenuToolReloadAssets(); } },
			},
			80,
			480);
	...
	void onClickMenuFileNew()
	{
		showConfirmSaveIfDirty(
			[this]
			{
				...
				m_assetDirectoryManuallySet = false;
	...
	bool loadFromFile(const FilePath& filePath, bool showMessageBoxOnError = true)
	{
		...
		// ファイルの場所に応じたカスタム定義を再読み込み
		m_customAssetLoader.reload(m_filePath);
		...
		// アセットのルートディレクトリを設定
		applyAssetBaseDirectory(filePath);
		m_assetDirectoryManuallySet = false;
	...
	bool onClickMenuFileSave()
	{
		...
		{
			const bool filePathChanged = (m_filePath != filePath);
			m_filePath = filePath;
			m_savedHash = json.formatMinimum().hash();
			if (filePathChanged)
			{
				// ファイルの場所が変わったのでカスタム定義を再読み込み
				m_customAssetLoader.reload(m_filePath);
	...
	void onClickMenuFileSaveAs()
	{
		if (const auto filePath = Dialog::SaveFile({ FileFilter{ U"NocoUI Canvas", { U"noco" } }, FileFilter::AllFiles() }))
		{
			...
			if (json.save(*filePath))
			{
				...
				{
					// 明示的にアセットパスが指定されていない場合はアセットのルートディレクトリを設定
					applyAssetBaseDirectory(*filePath);
				}
	...
	void onClickMenuToolChangeAssetDirectory()
	{
		if (Optional<String> pathOpt = Dialog::SelectFolder(noco::Asset::GetBaseDirectoryPath(), U"アセットのルートディレクトリを選択"))
		{
			...
			m_assetDirectoryManuallySet = true;
		}
	}

	void applyAssetBaseDirectory(const FilePath& filePath)
	{
		if (const auto configuredPath = CustomAssetLoader::ResolveAssetBaseDirectoryPath(filePath))
		{
			noco::Asset::SetBaseDirectoryPath(*configuredPath);
		}
		else
		{
			noco::Asset::SetBaseDirectoryPath(FileSystem::ParentPath(filePath));
		}
	}

	void onClickMenuToolReloadAssets()
	{
		// アセットキャッシュを全クリア(次回使用時に再読み込みされる)
		noco::Asset::UnloadAllTextures();
		noco::Asset::UnloadAllAudios();
		noco::Asset::UnloadAllJSONs();

		// 手動で設定されていない場合はアセットのルートディレクトリを再解決
		if (!m_assetDirectoryManuallySet && m_filePath)
		{
			applyAssetBaseDirectory(*m_filePath);
		}
