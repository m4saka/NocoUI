...
struct PropertyMetadata
{
	...
	std::function<void(Inspector*)> onChangeRefresh;  // 変更時にInspectorを更新
};
...
static HashTable<PropertyKey, PropertyMetadata> InitPropertyMetadata()
{
	...
	metadata[PropertyKey{ U"Sprite", U"nineSliceEnabled" }].onChangeRefresh = [](Inspector* inspector) { inspector->refreshInspector(); };
...
static HashTable<PropertyKey, String> InitPropertyTooltips()
{
	return HashTable<PropertyKey, String>
	{
		...
		{ PropertyKey{ U"Node", U"Active" }, U"Nodeの有効/無効状態" },
...
public:
	...
	std::shared_ptr<Node> createPropertyNodeWithTooltip(StringView componentName, StringView propertyName, StringView value, std::function<void(StringView)> fnSetValue, HasInteractivePropertyValueYN hasInteractivePropertyValue = HasInteractivePropertyValueYN::No)
	{
		...
		if (const auto it = m_propertyMetadata.find(PropertyKey{ String(componentName), String(propertyName) }); it != m_propertyMetadata.end())
		{
			...
			if (metadata.tooltip && const auto labelNode = propertyNode->getChildByNameOrNull(U"Label", RecursiveYN::Yes))
			{
	...
	std::shared_ptr<Node> createNodeNameNode(const std::shared_ptr<Node>& node)
	{
		...
		{
			const PropertyKey key{ U"Node", U"Active" };
	...
	std::shared_ptr<Node> createComponentNode(const std::shared_ptr<Node>& node, const std::shared_ptr<SerializableComponentBase>& component, IsFoldedYN isFolded, std::function<void(IsFoldedYN)> onToggleFold)
	{
		...
		for (const auto& property : component->properties())
		{
			...
			case PropertyEditType::Text:
				{
					auto onChange = [property](StringView value) { property->trySetPropertyValueString(value); };
					
					// メタデータに基づいて変更時の処理を設定
					const PropertyKey propertyKey{ component->type(), property->name() };
					if (const auto it = m_propertyMetadata.find(propertyKey); it != m_propertyMetadata.end())
					{
						const auto& metadata = it->second;
						if (metadata.onChangeRefresh)
						{
							onChange = [this, property, metadata](StringView value)
							{
								property->trySetPropertyValueString(value);
								metadata.onChangeRefresh(this);
							};
						}
					}
			...
			case PropertyEditType::Bool:
				{
					...
					if (const auto it = m_propertyMetadata.find(propertyKey); it != m_propertyMetadata.end())
					{
						...
						if (metadata.onChangeRefresh)
						{
							onChange = [this, property, metadata](bool value)
							{
			...
			case PropertyEditType::Vec2:
				{
					auto onChange = [property](const Vec2& value) { property->trySetPropertyValueString(Format(value)); };
					...
					if (const auto it = m_propertyMetadata.find(propertyKey); it != m_propertyMetadata.end())
					{
						...
						if (metadata.onChangeRefresh)
						{
							onChange = [this, property, metadata](const Vec2& value)
							{
								property->trySetPropertyValueString(Format(value));
								metadata.onChangeRefresh(this);
							};
			...
			case PropertyEditType::Color:
				{
					auto onChange = [property](const ColorF& value) { property->trySetPropertyValueString(Format(value)); };
					...
					if (const auto it = m_propertyMetadata.find(propertyKey); it != m_propertyMetadata.end())
					{
						...
						if (metadata.onChangeRefresh)
						{
							onChange = [this, property, metadata](const ColorF& value)
							{
								property->trySetPropertyValueString(Format(value));
								metadata.onChangeRefresh(this);
							};
			...
			case PropertyEditType::LRTB:
				{
					auto onChange = [property](const LRTB& value) { property->trySetPropertyValueString(Format(value)); };
					...
					if (const auto it = m_propertyMetadata.find(propertyKey); it != m_propertyMetadata.end())
					{
						...
						if (metadata.onChangeRefresh)
						{
							onChange = [this, property, metadata](const LRTB& value)
							{
								property->trySetPropertyValueString(Format(value));
								metadata.onChangeRefresh(this);
							};
