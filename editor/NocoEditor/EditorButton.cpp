...
namespace noco::editor
{
	std::shared_ptr<Node> CreateButtonNode(StringView text, const RegionVariant& region, std::function<void(const std::shared_ptr<Node>&)> onClick, IsDefaultButtonYN isDefaultButton)
	{
		...
		buttonNode->emplaceComponent<RectRenderer>(PropertyValue<Color>{ ColorF{ 0.1, 0.8 } }.withDisabled(ColorF{ 0.2, 0.8 }).withSmoothTime(0.05), PropertyValue<Color>{ ColorF{ 1.0, isDefaultButton ? 0.6 : 0.4 } }.withHovered(ColorF{ 1.0, isDefaultButton ? 0.8 : 0.6 }).withSmoothTime(0.05), 1.0, 0.0, 4.0);
