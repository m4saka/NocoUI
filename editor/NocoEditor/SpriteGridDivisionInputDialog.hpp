...
{
	class SpriteGridDivisionInputDialog : public IDialog
	{
	private:
	...
	public:
		explicit SpriteGridDivisionInputDialog(
			...
			, m_textureSize(0, 0)
		{
			const String texturePath = m_sprite->textureFilePath().defaultValue();
		...
		void createDialogContent(const std::shared_ptr<Node>& contentRootNode, const std::shared_ptr<ContextMenu>&, std::function<void()>) override
		{
			...
			columnsTextBox->setText(Format(m_sprite->textureGridColumns().defaultValue()));
			...
			const auto rowsTextBox = m_rowsTextBoxNode->emplaceComponent<TextBox>(
				...
				HorizontalAlign::Left, VerticalAlign::Middle,
				Palette::White, Color{ 255, 165, 0, 128 });
			rowsTextBox->setText(Format(m_sprite->textureGridRows().defaultValue()));
		...
		void onResult(StringView resultButtonText) override
		{
			if (resultButtonText == U"OK")
			{
				...
				m_sprite->setTextureGridCellSize(cellSize);
				m_sprite->setTextureGridColumns(columns);
				m_sprite->setTextureGridRows(rows);
